...
- **今日**：把今天待看的本季追番和补番任务分区显示，互不混排。
- **追番**：仅显示活动季度内 Bangumi“在看”的动画；展开番剧后才加载完整分集进度。
- **补番计划**：显示今天任务、未来七天、进行中、搁置和已完成的旧番。
- **想看**：显示 Bangumi“想看”动画，可按名称和年份筛选。
- **每日放送**：显示放送日期、具体时间和数据来源，并可保存或恢复本地时间校正。

搜索和加入想看、加入补番等收藏操作会先更新页面，再由后台串行写入 Bangumi 并同步本地缓存。页面不会等待完整同步；同步失败会保留缓存并显示可重试错误。
...
提前放送和预览不作为季度起点。日本 `25:00` 等 30 小时制时间会换算为上海时区次日 `00:00`，再参与星期、日期和每日负载计算。
...
3. 启动应用，在追番页底部的“设置”中填写 Bangumi App ID 和 App Secret。
...
最简单的方式是在 Finder 中双击项目根目录的：

```text
Start Bangumi Watch Planner.command
```

它会构建当前代码、安装或更新 LaunchAgent、等待服务启动，然后打开 `http://127.0.0.1:3777/`。以后登录 macOS 后服务会自动运行。
...
每日放送优先采用 ACG Secrets，缺失时依次使用 Bangumi Index、Bangumi Data 和 Bangumi 原始日历。每条记录会显示当前来源。

点击“校正时间”可直接修改日期和小时。应用会把日期差保存为整部番的偏移，因此 `25:00` 跨日、提前或推迟一周等修正会用于后续分集同步；“恢复来源”会删除本地校正。
