...
- 标记今天任务已看后会清理对应任务，但当天不再补位；剩余集数只进入明天及之后。
- 只有主动点击“重新规划今天”才会再次为当天安排任务。
...
首次打开生产服务时，页面会要求设置至少 8 个字符的访问密码。此密码用于保护本机和局域网访问，与 Bangumi 账号密码无关。

- 密码只以 `scrypt` 哈希保存在本地 SQLite。
- 登录会话保存在 HttpOnly Cookie 中，有效期 30 天，服务重启后仍有效。
- 可在追番提醒底部的“设置”中退出登录。
- 未登录时无法读取番剧、计划和设置，也无法进入 Bangumi OAuth。
...
假设输出 `192.168.1.23`，同一局域网设备可打开 `http://192.168.1.23:3777/`，然后使用同一个访问密码登录。如只允许本机访问，在 `.env.local` 设置：
